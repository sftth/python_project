...
from naver_stock.utils.file_utils import generate_dated_excel_filename, save_excel,save_excel_v1, generate_dated_excel_filename_monthly
...
def get_stock_info_v1(stock_list: list, prefix="nh", output_dir="/output/"):
    # 데이터 저장용 리스트 생성
    all_stock_data = []
    # 각 종목별 데이터 요청
    headers = {"User-Agent": "Mozilla/5.0"}

    # market: domestic / worldstock
    # type: etf, stock
    # name: "애플"
    # stock_code: ticker
    for market, category, name, stock_code in stock_list:
        ...
        if response.status_code == 200:
            data = response.json()
            stock_info_list = get_realtime_data_v1(data['datas'], firm=prefix.upper(), category=category, name=name, stock_code=stock_code)

        all_stock_data += additional_infos_v1(stock_info_list, stock_code, market=market)

    save_info_to_excel_v1(all_stock_data, prefix=prefix, output_dir=output_dir)
...
def get_realtime_data_v1(data, firm, category, name, stock_code):
    # 운영체제에 따라 날짜 포맷 선택
    if platform.system() == "Windows":
        date_format = "%Y.%#m.%#d"
    else:  # macOS, Linux
...
def additional_infos_v1(stock_info_list,stock_code, market="domestic"):
    # 각 종목별 데이터 요청
    headers = {"User-Agent": "Mozilla/5.0"}
    try:
        # Assign url for domestic or worldstock
        if market == 'domestic':
            ...
            if response_basic.status_code == 200:
                ...
                for stock_info in stock_info_list:
                    for code_key in target_codes:
                        label, need_float = target_codes[code_key]
                        value = "" # 기본값은 공백 처리

                        # 데이터가 존재하고 해당 key가 있으면  처리
                        for item in data.get("stockItemTotalInfos", []):
                            if item["code"] ==  code_key:
                                value = clean_and_convert(item["value"], need_float)
                                break

                        stock_info[label] = value # 값 설정
            # 목표주가 처리
            if response_additional.status_code == 200:
                data = response_additional.json()
                if data['consensusInfo'] is not None:
...
def save_info_to_excel_v1(all_stock_data, prefix="nh", output_dir="/output"):
    # 데이터프레임 변환
    df = pd.DataFrame(all_stock_data)
    ...
    sheet_name = f"{prefix}_{today_str}"

    excel_filename = generate_dated_excel_filename_monthly(prefix=prefix, output_dir=output_dir)
    save_excel_v1(df, excel_filename, sheet_name=sheet_name)
    #excel_filename = "nh_total_stocks_data_v1.0.xlsx"
    #df.to_excel(excel_filename, index=False, sheet_name="Stock TotalInfos")
...
#        row[4]
        ...
        row[14].style = number_style
        row[15].style = number_style
        row[16].style = number_style
        row[17].style = number_style
        row[18].style = percent_style
        row[19].style = number_style
        row[20].style = number_style
    # ✅ 엑셀 파일 저장
    wb.save(excel_filename)
